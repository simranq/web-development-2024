...
function selectGameField(event) {
  ...
  const selectedColumn = selectedField.dataset.col - 1; //for array we've subtracted
  const selectedRow = selectedField.dataset.row - 1; //also, string is thus converted into no.,thus, no '+' req!
...
function checkForGameOver() {
  ...
  //column check for winner, we can use 'i' here but for my uderstanding, I've used the convention of my own.
  for (let j = 0; j < 3; j++) {
    if (
      gameData[0][j] > 0 &&
  ...
  //Diagonal check top left -to- ryt bottom
  if (
    gameData[0][0] > 0 &&
  ...
  //Diagonal check bottom left -to- top ryt
  if (
    gameData[2][0] > 0 &&
